...
void TLD::init(const Mat& frame1,const Rect& box,FILE* bb_file){
  ...
  //Get Bounding Boxes
	...
	fprintf(log_file,"Created %d bounding boxes\n",(int)grid.size());
  ///Preparation
  //allocation
  ...
  fprintf(log_file,"Found %d good boxes, %d bad boxes\n",(int)good_boxes.size(),(int)bad_boxes.size());
  fprintf(log_file,"Best Box: %d %d %d %d\n",best_box.x,best_box.y,best_box.width,best_box.height);
  fprintf(log_file,"Bounding box hull: %d %d %d %d\n",bbhull.x,bbhull.y,bbhull.width,bbhull.height);
...
void TLD::generatePositiveData(const Mat& frame, int num_warps){
  ...
  fprintf(log_file,"Positive examples generated: ferns:%d NN:1\n",(int)pX.size());
}
 ...
 */
  ...
  fprintf(log_file,"negative data generation started.\n");
  vector<int> fern(classifier.getNumStructs());
  ...
  fprintf(log_file,"Negative examples generated: ferns: %d ",a);
...
void TLD::processFrame(const cv::Mat& img1,const cv::Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file){
  ...
  printf("Track time:%.6f\n",(getTickCount()-timer)/getTickFrequency());
  fprintf(log_file,"Track time:%.6f\n",(getTickCount()-timer)/getTickFrequency());
  ///Detect
  timer=getTickCount();
  if (frame_count%1==0)
	detect(img2);
  ...
  printf("Detect time:%.6f\n",(getTickCount()-timer)/getTickFrequency());
  fprintf(log_file,"Detect time:%.6f\n",(getTickCount()-timer)/getTickFrequency());
  ///Integration
  timer=getTickCount();
  if (tracked){
      bbnext=tbb;
	  ...
	  if(detected){                                               //   if Detected
          ...
          if (confident_detections==1){                                //if there is ONE such a cluster, re-initialize the tracker
              ...
              fprintf(log_file,"Found a better match..reinitializing tracking\n");
              bbnext=cbb[didx];
              lastconf=cconf[didx];
              lastvalid=false;
          }
              ...
              fprintf(log_file,"%d confident cluster was found\n",confident_detections);
              ...
              for (int i=0;i<dbb.size();i++){
                  if(bbOverlap(tbb,dbb[i])>0.7){                     // Get mean of close detections
					  ...
					  fprintf(log_file,"weighted detection: %d %d %d %d\n",dbb[i].x,dbb[i].y,dbb[i].width,dbb[i].height); 
				  }
              }
              if (close_detections>0){
				  ...
				  fprintf(log_file,"Tracker bb: %d %d %d %d\n",tbb.x,tbb.y,tbb.width,tbb.height);
                  fprintf(log_file,"Average bb: %d %d %d %d\n",bbnext.x,bbnext.y,bbnext.width,bbnext.height);
                  fprintf(log_file,"Weighting %d close detection(s) with tracker..\n",close_detections);
              
			  }
              else{
                printf("%d close detections were found\n",close_detections);
				fprintf(log_file,"%d close detections were found\n",close_detections);
              }
          }
      }
  }
      ...
      if(detected){                           //  and detector is defined
          ...
          if (cconf.size()==1){
              ...
              fprintf(log_file,"Confident detection..reinitializing tracker\n");
              lastboxfound = true;
          }
      }
  }
  printf("Integration time:%.6f\n",(getTickCount()-timer)/getTickFrequency());
  fprintf(log_file,"Integration time:%.6f\n",(getTickCount()-timer)/getTickFrequency());
  ...
  if (lastvalid && tl)
    learn(img2);
  printf("Learn time:%.6f\n",(getTickCount()-timer)/getTickFrequency());
  fprintf(log_file,"Learn time:%.6f\n",(getTickCount()-timer)/getTickFrequency());
}
...
void TLD::track(const Mat& img1, const Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2){
  ...
  if (points1.size()<1){
      ...
      fprintf(log_file,"BB= %d %d %d %d, Points not generated\n",lastbox.x,lastbox.y,lastbox.width,lastbox.height);
      tvalid=false;
      tracked=false;
      return;
  }
  ...
  if (tracked){
      ...
      if (tracker.getFB()>10 || tbb.x>img2.cols ||  tbb.y>img2.rows || tbb.br().x < 1 || tbb.br().y <1){
          ...
          fprintf(log_file,"Too unstable predictions FB error=%f\n",tracker.getFB());
          return;
      }
	...
	fprintf(log_file,"No points tracked\n");  
  }
}
...
void TLD::bbPoints(vector<cv::Point2f>& points,const BoundingBox& bb){
  ...
  int margin_h=0;
  int margin_v=0;
  int stepx = cvCeil((bb.width-2*margin_h)/max_pts);
  int stepy = cvCeil((bb.height-2*margin_v)/max_pts);
...
void TLD::bbPredict(const vector<cv::Point2f>& points1,const vector<cv::Point2f>& points2,
  ...
  fprintf(log_file,"tracked points : %d\n",npoints);
  ...
  fprintf(log_file,"s= %f s1= %f s2= %f \n",s,s1,s2);
  ...
  fprintf(log_file,"predicted bb: %d %d %d %d\n",bb2.x,bb2.y,bb2.br().x,bb2.br().y);
}
...
void TLD::detect(const cv::Mat& frame){
  ...
  fprintf(log_file,"%d Bounding boxes passed the variance filter\n",a);
  fprintf(log_file,"%d Initial detection from Fern Classifier\n",detections);
  if (detections>100){
...
//  imshow("detections",img);
  ...
  fprintf(log_file,"Fern detector made %d detections ",detections);
  t=(double)getTickCount()-t;
  printf("in %gms\n", t*1000/getTickFrequency());
  fprintf(log_file,"in %gms\n", t*1000/getTickFrequency());
  ...
  if (dbb.size()>0){
      ...
      fprintf(log_file,"Found %d NN matches\n",(int)dbb.size());
      detected=true;
  }
  else{
      printf("No NN matches found.\n");
      fprintf(log_file,"No NN matches found.\n");
      detected=false;
  }
}
...
void TLD::learn(const Mat& img){
  ...
  if (conf<0.5) {
      ...
      fprintf(log_file,"Fast change..not training\n");
      lastvalid =false;
      return;
  }
  if (pow(stdev.val[0],2)<var){
      printf("Low variance..not training\n");
      fprintf(log_file,"Low variance..not training\n");
      lastvalid=false;
      return;
  }
  if(isin[2]==1){
      printf("Patch in negative data..not traing");
      fprintf(log_file,"Patch in negative data..not traing");
      lastvalid=false;
      return;
  }
/// Data generation
  ...
  if (good_boxes.size()>0)
    ...
    fprintf(log_file,"No good boxes..Not training");
    return;
  }
...
void TLD::clusterConf(const vector<BoundingBox>& dbb,const vector<float>& dconf,vector<BoundingBox>& cbb,vector<float>& cconf){
  ...
  fprintf(log_file,"Cluster indexes: ");
  BoundingBox bx;
  for (int i=0;i<c;i++){
      float cnf=0;
