#ifndef _CONFIG_H
#define _CONFIG_H
#include"common.h"
#include "detecting.h"

class Config{
public:
	Mat bb;
	vector<Mat> img;//img[I]表示这个点的图。


	Mat xFJ;
	Size imgsize;
	struct Model{
		Size patchsize;
		float thr_nn_valid;
		float ncc_thesame;
		float valid;  
		float thr_fern; 
		float thr_nn;
		int num_trees;
		int num_features;
		int min_win; 
		int fliplr;
	} model;
	vector<int> valid;
	Mat pex;
	Mat nex;
	struct Control{
		float maxbbox; //fraction of evaluated bounding boxes in every frame, maxbox = 0 means detector is truned off, if you don't care about speed set it to 1 
		int update_detector; //% online learning on/off, of 0 detector is trained only in the first frame and then remains fixed
		int drop_img; // drop_img = 1;
		int repeat; // repeat=1;
	} control;

	struct Tmp{
		float patt;
		float conf;
	} tmp;

	vector<DT> dt;

	Mat grid; // store bounding box;
	Mat scales; // 
	Mat feature; // 鍘熸潵閲岄潰瀹氫箟鐨刦eatures杩樻湁涓垚鍛樻槸type, default鏄?forest", 涓嶇煡閬撴湁娌℃湁鐢紝涓轰簡鏂逛究鍏堝畾涔変负mat;
	float var; //  Variance threshold
	vector<float> tmpConf; // 鍘熸潵鐨則ld.tmp.conf
	Mat tmpPatt; // 鍘熸潵鐨則ld.tmp.patt

};
#endif
